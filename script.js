...
let RENDER_DISTANCE = 30; // nanti otomatis update sesuai layar
...
function updatePlayer(){
  ...
  if(Math.abs(player.vx)>0 && player.grounded){
    animTimer++;
    if(animTimer>WALK_SPEED){ animFrame = (animFrame+1)%4; animTimer=0; }
  }else animFrame=0;
...
function updateMining(){
  if(miningBlock){
    ...
    if(miningProgress>=MINING_SPEED){
      ...
      if(type>0) addToInventory(type,1);
      world[x][y] = 0;
      miningBlock = null;
      miningProgress = 0;
    }
...
function drawPlayer(){
  ...
  ctx.fillStyle="#ffcc99"; ctx.fillRect(x+4,y,12,10);
  ctx.fillStyle="#00aaaa"; ctx.fillRect(x+3,y+10,14,10);
...
function drawInventory(){
  for(let i=0;i<player.inventory.length;i++){
    ...
    if(item.type===1) ctx.drawImage(textures.dirt,x,y);
    if(item.type===2) ctx.drawImage(textures.grass,x,y);
    if(item.type===3) ctx.drawImage(textures.wood,x,y);
    if(item.type===4) ctx.drawImage(textures.leaves,x,y);
...
function resizeCanvas(){
  ...
  RENDER_DISTANCE = Math.ceil(canvas.width/TILE)+2;
}
